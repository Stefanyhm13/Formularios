...
def obtener_respuestas_procesadas1(respuestas_totales):
    """
    Convierte la lista 'respuestas_totales' en un diccionario {pregunta: respuesta}.
    Devuelve None si 'respuestas_totales' es None.
    """
    if respuestas_totales is None:
        print("respuestas_totales es None en obtener_respuestas_procesadas1. Retornando None.")
        return None
...
def funcion_procesarb(cuestionarios):

    if len(cuestionarios) <= 1 or cuestionarios[1] is None:
        return None

    ruta_cuestionario = cuestionarios[1]
