...
def obtener_respuestas_procesadas4(respuestas_totales):
...
def funcion_procesares(cuestionarios):
    
    if len(cuestionarios) <= 1 or cuestionarios[3]  is None:
        raise FileNotFoundError("El cuestionario Intralaboral no fue encontrado.")

    ruta_cuestionario = cuestionarios[3]
...
def procesar_cuestionario_estres(tipo_empleado,respuestas_procesadas):
    respuestas_procesadas = obtener_respuestas_procesadas4(respuestas_procesadas)
    ...
    return list((puntaje_bruto, puntaje_transformado, clasificacion))
